...
function fogmodifier.create(p, where, see, share, over)
	...
	if where.type == 'rect' then
		j_handle = jass.CreateFogModifierRect(p.handle, jass.ConvertUnitState(see), rect.j_temp(where), share, over)
	elseif where.type == 'circle' then
		local x, y, r = where:get()
		j_handle = jass.CreateFogModifierRadius(p.handle, jass.ConvertUnitState(see), x, y, r, share, over)
	end
