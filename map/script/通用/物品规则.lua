...
ac.game:event '单位-即将获得物品'(function(_,u,it)
    ...
    if u:get_type_count(it) then
        u:get_owner():sendMsg('|cffebb608【系统】|cff00ff00已拥有相同装备')
        --回收句柄
        it.recycle = false
        return true
    end
...
ac.game:event '单位-点击商店物品'(function(_,seller,u,it)
    ...
    fire_seeds = (it.player_fire_seed and it.player_fire_seed[player]) or fire_seeds
    ...
    if gold < golds and not has_raffle then
        u:get_owner():sendMsg('|cffebb608【系统】|r|cffff0000钱不够',3)
        ac.game:event_notify('单位-货币不足',seller,u,it)
        return
    end 
    if wood < woods and not has_raffle  then
        u:get_owner():sendMsg('|cffebb608【系统】|r|cffff0000木头不够',3)
        ac.game:event_notify('单位-货币不足',seller,u,it)
        return
    end
    if kill_count < kill_counts and not has_raffle then
        u:get_owner():sendMsg('|cffebb608【系统】|r|cffff0000杀敌数不够',3)
        ac.game:event_notify('单位-货币不足',seller,u,it)
        return
    end
    if jifens > 0 and not has_raffle then 
        if jifen < jifens then
            u:get_owner():sendMsg('|cffebb608【系统】|r|cffff0000积分不够',3)
            ac.game:event_notify('单位-货币不足',seller,u,it)
            return
        end
    end   
    if fire_seed < fire_seeds and not has_raffle then
        u:get_owner():sendMsg('|cffebb608【系统】|r|cffff0000火灵不够',3)
        ac.game:event_notify('单位-货币不足',seller,u,it)
        return
    end 
    ...
    if it.raffle then
        if  fire_seeds == 0 and jifens == 0 and kill_counts == 0 and woods == 0 and golds == 0  and not has_raffle then
            u:get_owner():sendMsg('|cffebb608【系统】|r|cffff0000条件不符',3)
            ac.game:event_notify('单位-货币不足',seller,u,it)
            return
        end 
    end    
    ...
    if not it.player_buy_cnt[player] then 
        it.player_buy_cnt[player] = 0
    end
    ...
    if u.buy_suc then 
        --记录每个玩家的购买次数
        -- print('记录log1',it.player_buy_cnt,player)
        -- print('记录log2',it.player_buy_cnt[player])
